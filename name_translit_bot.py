...
async def start(message: Message):
    await message.reply(
        "Привет! Отправьте мне ваше ФИО на русском языке, и я переведу его на латиницу в соответствии с Приказом МИД России от 12.02.2020 № 2113."
    )
...
async def translit_fio(message: Message):
    ...
    logging.info(
        f"Пользователь {message.from_user.username} отправил ФИО: {fio}, транслитерация: {translit_fio}"
    )
